...
extern struct SancusModule aesdust;

  #ifdef ASM
    // s should point to 128-bit data and 128-bit key
   __attribute__((eentry)) void aes_ecb(__attribute__((secret)) void *s);
  #else
  // mk should point to a 128-bit or 256-bit key
  // data should point to a 128-bit block of plaintext to encrypt
  __attribute__((eentry)) void aes_ecb(__attribute__((secret)) void *mk,
                                       __attribute__((secret)) void *data);
  #endif
  ...
  __attribute__((eentry)) void aes_ctr(u32 len,
                                       void *ctr,
                                       __attribute__((secret)) void *data,
                                       __attribute__((secret)) void *mk); 
  
  // iv is the 128-bit initialisation vector.
  __attribute__((eentry)) void aes_ofb(u32 len,
                                       void *iv,
                                       __attribute__((secret)) void *data,
                                       __attribute__((secret)) void *mk); 
